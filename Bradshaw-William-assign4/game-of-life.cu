...
__global__ void compute_on_gpu(int *life, int *previous_life, int X_limit, int Y_limit) {
    ...
    for (int i = i0; i < X_limit * Y_limit; i += stride) {
        int padx = (i / Y_limit) + 1;
        int pady = (i % Y_limit) + 1;
        int x = i / Y_limit;
        int y = i % Y_limit;
        
        neighbors = previous_life[(padx-1)*(Y_limit+2) + (pady-1)] + previous_life[(padx-1)*(Y_limit+2) + (pady)] +
                    previous_life[(padx-1)*(Y_limit+2) + (pady+1)] + previous_life[(padx)*(Y_limit+2) + (pady-1)] +
                    previous_life[(padx)*(Y_limit+2) + (pady+1)] + previous_life[(padx+1)*(Y_limit+2) + (pady-1)] +
                    previous_life[(padx+1)*(Y_limit+2) + (pady)] + previous_life[(padx+1)*(Y_limit+2) + (pady+1)];

        if (previous_life[padx*(Y_limit+2) + pady] == 0) {
...
/* I'm not totally sure why, but both of the copy methods were completed in my files. */
/* This is likely because I pulled these prior to them being removed, but we take those :3 */
...
__global__ void padded_matrix_copy(int *dst, int *src, int width, int height, int padding) {
    ...
    for (int i = i0; i < width * height; i += stride) {
        int x = i / height;
        int y = i % height;
        
        dst[(x + padding) * (height + (2 * padding)) + (y + padding)] = src[i];
    }
}
